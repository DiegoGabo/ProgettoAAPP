...
Atomicwrap::Atomicwrap() noexcept{
	HashEntry he;
	std::atomic<HashEntry> ahe(he);
}
...
void HashTable::incrementValue(std::vector<Nucleotide> key){
	...
	do{
		...
		i++;
		done = std::atomic_compare_exchange_strong(&table[pos].ahe,&empty_he,new_he); /*CAS*/
	}while(!done || !(equal(key,table[pos].ahe.load(std::memory_order_relaxed).getK())) || i>=10); /*esco anche se key non è vuota ma il confronto della chiave da successo*/
	int oldval = table[pos].ahe.load(std::memory_order_relaxed).getC();
	done = false;
	do{
		oldval = table[pos].ahe.load(std::memory_order_relaxed).getC();
		...
		done = std::atomic_compare_exchange_strong(&table[pos].ahe,&old_he,new_he); /*CAS*/
	}while(!done);
